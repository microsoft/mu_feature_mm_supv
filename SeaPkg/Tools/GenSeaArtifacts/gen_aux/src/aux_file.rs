...
impl AuxFile {
    pub fn add_entry(&mut self, entry: ImageValidationEntryHeader, raw_data: &[u8]) {
        assert_eq!(entry.size as usize, raw_data.len(), "Default value raw data must match the size being validated.");
...
impl <'a> TryIntoCtx<Endian> for &AuxFile {
...
impl <'a> TryIntoCtx<Endian> for &ValidationType {
    type Error = scroll::Error;
    fn try_into_ctx(self, this: &mut [u8], ctx: Endian) -> Result<usize, Self::Error> {
        let mut offset = 0;

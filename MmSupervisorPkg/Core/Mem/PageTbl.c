...
//
// Splittable helper.  PageTbl_core.c patches the prebuilt SMI-entry asm thunk's
// 5-level-paging slot when the supervisor itself owns the SMI entry handler;
// PageTbl_init.c provides a no-op since the init driver never runs an SMI.
//
VOID
  ...
  )
{
  ...
  SmiPagingPatch5LevelHook (m5LevelPagingNeeded);

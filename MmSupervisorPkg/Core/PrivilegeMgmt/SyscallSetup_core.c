/** @file
  Core (MmSupervisorCore) syscall MSR setup helpers.

  These helpers are linked only into the runtime MmSupervisorCore driver and
  perform per-CPU MSR programming for SYSCALL/SYSRET, plus the BSP-routed
  variants that consume mSmmMpSyncData.  MmSupervisorInit does not run any of
  these and therefore omits this translation unit.

  See SyscallSetup.c for the file-scope MSR-store globals and the routines
  that are linked into both drivers (e.g., GetThisCpl3Stack and
...
//
// Defined in SyscallSetup.c (shared).
//
extern UINT64                 *mMsrStarStore;
extern UINT64                 *mMsrStar64Store;
extern UINT64                 *mMsrEferStore;
extern MM_SUPV_SYSCALL_CACHE  *mMmSupvGsStore;

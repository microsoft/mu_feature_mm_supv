...
extern LIST_ENTRY  mMmiEntryList;
extern LIST_ENTRY  mHardwareSmiEntryList;
extern MMI_ENTRY   mRootMmiEntry;
...
GLOBAL_REMOVE_IF_UNREFERENCED LIST_ENTRY  *mSmmCoreSmiEntryList         = &mMmiEntryList;
...
GLOBAL_REMOVE_IF_UNREFERENCED USER_HANDLER_REG_STRUCT  UserHandlerRegHolder;
  ...
  )
{
  ...
  CHAR16                     *PathStr;
  EFI_MM_DRIVER_ENTRY        *LoadedImagePrivate;
  PHYSICAL_ADDRESS           EntryPoint;
  ...
  HandleBufferSize = 0;
  HandleBuffer     = NULL;
  Status           = gMmCoreMmst.MmLocateHandle (
                                   ByProtocol,
  ...
  for (Index = 0; Index < NoHandles; Index++) {
    Status = gMmCoreMmst.MmHandleProtocol (
                           HandleBuffer[Index],
                           &gEfiLoadedImageProtocolGuid,
                           (VOID **)&LoadedImage
                           );
    ...
    FindFileNameFromDiscoveredList ((EFI_PHYSICAL_ADDRESS)LoadedImage->ImageBase, &Guid);
    DEBUG ((DEBUG_INFO, "Image: %g ", &Guid));

    EntryPoint         = 0;
    LoadedImagePrivate = BASE_CR (LoadedImage, EFI_MM_DRIVER_ENTRY, LoadedImage);
    RealImageBase      = (UINTN)LoadedImage->ImageBase;
    if (LoadedImagePrivate->Signature == EFI_MM_DRIVER_ENTRY_SIGNATURE) {
...
DumpSmiHandlerOnSmiEntry (
  IN MMI_ENTRY  *SmiEntry
  )
{
  LIST_ENTRY    *ListEntry;
  MMI_HANDLER   *SmiHandler;
  IMAGE_STRUCT  *ImageStruct;

  ListEntry = &SmiEntry->MmiHandlers;
  for (ListEntry = ListEntry->ForwardLink;
       ListEntry != &SmiEntry->MmiHandlers;
       ListEntry = ListEntry->ForwardLink)
  {
    SmiHandler  = CR (ListEntry, MMI_HANDLER, Link, MMI_HANDLER_SIGNATURE);
  ...
  )
{
  ...
  for (ListEntry = ListEntry->ForwardLink;
       ...
       ListEntry = ListEntry->ForwardLink)
  {
    SmiEntry = CR (ListEntry, MMI_ENTRY, AllEntries, MMI_ENTRY_SIGNATURE);
  ...
  )
{
  ...
  // Sanity check
  //
  if (!MmIsBufferOutsideMmValid ((UINTN)SmiHandlerProfileGetDataByOffset.DataBuffer, (UINTN)SmiHandlerProfileGetDataByOffset.DataSize)) {
  ...
  )
{
  ...
  Status = gMmCoreMmst.MmiHandlerRegister (
                         SmiHandlerProfileHandler,
  ...
  )
{
  ...
  if ((SmiEntry == NULL) && Create) {
    SmiEntry = AllocatePool (sizeof (MMI_ENTRY));
    if (SmiEntry != NULL) {
      //
      // Initialize new SMI entry structure
      //
      SmiEntry->Signature = MMI_ENTRY_SIGNATURE;
      CopyGuid ((VOID *)&SmiEntry->HandlerType, HandlerType);
      InitializeListHead (&SmiEntry->MmiHandlers);
  ...
  )
{
  ...
  ASSERT (SmiHandlerUsbContextSize != NULL);

  DevicePathSize = GetDevicePathSize (UsbContext->Device);
  Status         = InspectTargetRangeOwnership ((EFI_PHYSICAL_ADDRESS)(UINTN)UsbContext->Device, DevicePathSize, &IsUserRange);
  ...
  )
{
  ...
  SmiHandler = AllocateZeroPool (sizeof (MMI_HANDLER));
  if (SmiHandler == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  SmiHandler->Signature   = MMI_HANDLER_SIGNATURE;
  ...
  List = &SmiEntry->MmiHandlers;

  SmiHandler->MmiEntry = SmiEntry;
...
/**
  ...
  @retval EFI_SECURITY_VIOLATION  Supplied context buffer is not a user buffer.
  @retval Others                  Other errors returned from SmiHandlerProfileRegisterHandler
                                  or InspectTargetRangeOwnership.
**/
EFI_STATUS
  ...
  )
{
  ...
  switch (SyscallIndex) {
    case SMM_MM_HDL_REG_1:
      ZeroMem (&UserHandlerRegHolder, sizeof (UserHandlerRegHolder));
      UserHandlerRegHolder.HandlerGuid      = HandlerGuid;
      UserHandlerRegHolder.Handler          = (EFI_SMM_HANDLER_ENTRY_POINT2)Arg2;
      UserHandlerRegHolder.CallerAddress    = Arg3;
      UserHandlerRegHolder.CompletedSyscall = SyscallIndex;
      break;
    case SMM_MM_HDL_REG_2:
      if ((UserHandlerRegHolder.CompletedSyscall != SMM_MM_HDL_REG_1) ||
          (CompareMem (UserHandlerRegHolder.HandlerGuid, HandlerGuid, sizeof (EFI_GUID)) != 0))
      {
      ...
      UserHandlerRegHolder.ContextSize      = (UINTN)Arg3;
      UserHandlerRegHolder.CompletedSyscall = SyscallIndex;

      // Inspect ownership of Context region, if not NULL, before passing into registration
      if (UserHandlerRegHolder.Context != NULL) {
        Status = InspectTargetRangeOwnership (
                   (EFI_PHYSICAL_ADDRESS)(UINTN)UserHandlerRegHolder.Context,
      ...
      Status = SmiHandlerProfileRegisterHandler (
                 &mSmiHandlerProfile,
                 UserHandlerRegHolder.HandlerGuid,
                 UserHandlerRegHolder.Handler,
                 UserHandlerRegHolder.CallerAddress,
  ...
  )
{
  ...
  Head             = &SmiEntry->MmiHandlers;
  for (Link = Head->ForwardLink; Link != Head; Link = Link->ForwardLink) {
    SmiHandler = CR (Link, MMI_HANDLER, Link, MMI_HANDLER_SIGNATURE);
  ...
  if (IsListEmpty (&SmiEntry->MmiHandlers)) {
  ...
  )
{
  ...
  switch (SyscallIndex) {
    ...
    case SMM_MM_HDL_UNREG_2:
      if ((UserHandlerRegHolder.CompletedSyscall != SMM_MM_HDL_UNREG_1) ||
          (CompareMem (UserHandlerRegHolder.HandlerGuid, HandlerGuid, sizeof (EFI_GUID)) != 0))
      {
        Status = EFI_NOT_STARTED;
        goto Exit;
  ...
  )
{
  ...
  if ((PcdGet8 (PcdSmiHandlerProfilePropertyMask) & 0x1) != 0) {
    InsertTailList (&mRootSmiEntryList, &mRootMmiEntry.AllEntries);

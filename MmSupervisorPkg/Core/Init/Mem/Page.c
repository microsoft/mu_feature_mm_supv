...
/**
  ...
  @param[in]   NeedGuard              Flag to indicate Guard page is needed
                                      or not
  @param[in]   SupervisorPage         True as this is requesting to apply attribute of supervisor pages
  ...
  )
{
  ...
  Entry->Start            = Start;
  Entry->End              = End;
  Entry->IsSupervisorPage = SupervisorPage;
  if (Next) {
  ...
  )
{
  ...
  while (Link != &gMemoryMap) {
    ...
    //
    if (Entry->Start > End) {
      if ((Entry->Start == End + 1) && (Entry->Type == Type) && (Entry->IsSupervisorPage == SupervisorPage)) {
        Entry->Start = Start;
        return;
      }
    ...
    if ((Entry->Start <= Start) && (Entry->End >= End)) {
      if ((Entry->Type != Type) || (Entry->IsSupervisorPage != SupervisorPage)) {
        ...
        if (NextLink != &gMemoryMap) {
          ...
          //    +----------+   +------+   +-----------------+
          //
          if ((Entry->Type == NextEntry->Type) && (Entry->IsSupervisorPage == NextEntry->IsSupervisorPage) && (Entry->End + 1 == NextEntry->Start)) {
        ...
        if (PreviousLink != &gMemoryMap) {
          ...
          //    +----------+   +-----------------+   +------+
          //
          if ((PreviousEntry->Type == Entry->Type) && (PreviousEntry->IsSupervisorPage == Entry->IsSupervisorPage) && (PreviousEntry->End + 1 == Entry->Start)) {
  ...
  if (Link != &gMemoryMap) {
    Entry = CR (Link, MEMORY_MAP, Link, MEMORY_MAP_SIGNATURE);
    if ((Entry->End + 1 == Start) && (Entry->Type == Type) && (Entry->IsSupervisorPage == SupervisorPage)) {
      Entry->End = End;
      return;
    }
  }
  ...
  )
{
      ...
      if (NeedGuard) {
        *Memory = InternalAllocMaxAddressWithGuard (
        ...
        if (*Memory == (UINTN)-1) {
          ...
          // The page guard pages are handled separately. Memory here will be all that we need.
          goto ApplyPageAttributes;
        }
      }
  ...
  //
  ConvertMmMemoryMapEntry (MemoryType, *Memory, NumberOfPages, AddRegion, SupervisorPage);
  if (!AddRegion) {
    CoreFreeMemoryMapStack ();
  }
...
ApplyPageAttributes:
  if (mCoreInitializationComplete) {
    if (MemoryType == EfiRuntimeServicesCode) {
      SmmClearMemoryAttributes (*Memory, EFI_PAGES_TO_SIZE (NumberOfPages), EFI_MEMORY_XP);
      // We should only allow supervisor initiated request propagate through here
      SmmClearMemoryAttributes (*Memory, EFI_PAGES_TO_SIZE (NumberOfPages), EFI_MEMORY_RO);
    } else {
      // EfiRuntimeServicesData
      SmmClearMemoryAttributes (*Memory, EFI_PAGES_TO_SIZE (NumberOfPages), EFI_MEMORY_RO);
      SmmSetMemoryAttributes (*Memory, EFI_PAGES_TO_SIZE (NumberOfPages), EFI_MEMORY_XP);
    }

    if (SupervisorPage) {
      SmmSetMemoryAttributes (*Memory, EFI_PAGES_TO_SIZE (NumberOfPages), EFI_MEMORY_SP);
    } else {
      // EfiRuntimeServicesData
      SmmClearMemoryAttributes (*Memory, EFI_PAGES_TO_SIZE (NumberOfPages), EFI_MEMORY_SP);
    }
  }
  ...
  )
{
  ...
  // Before freeing pages, verify the candidate attributes not crossing boundary between user and supervisor
  if (mCoreInitializationComplete) {
    Status = SmmGetMemoryAttributes (Memory, EFI_PAGES_TO_SIZE (NumberOfPages), &Attributes);
    if (EFI_ERROR (Status) ||
        (((Attributes & EFI_MEMORY_SP) != 0) && !SupervisorPage) ||
        (((Attributes & EFI_MEMORY_SP) == 0) && SupervisorPage))
    {
      ASSERT_EFI_ERROR (Status);
      return Status;
  ...
  // Update MmMemoryMap here.
  //
  ConvertMmMemoryMapEntry (EfiConventionalMemory, Memory, NumberOfPages, AddRegion, TRUE);
  if (!AddRegion) {
    CoreFreeMemoryMapStack ();
  }
...
{
  if (IsGuarded) {
    return MmInternalFreePagesExWithGuard (Memory, NumberOfPages, FALSE, SupervisorPage);
  }

  return MmInternalFreePagesEx (Memory, NumberOfPages, FALSE, SupervisorPage);
}

/**
  Check whether the input range is in memory map.

  @param  Memory                 Base address of memory being passed in.
  @param  NumberOfPages          The number of pages.
  @param  SupervisorPage         Placeholder for flag of this page ownership.
  ...
  )
{
  ...
  while (Link != &gMemoryMap) {
    ...
    if ((Entry->Start <= Memory) && (Entry->End >= Last)) {
      *SupervisorPage = Entry->IsSupervisorPage;
      return TRUE;
    }
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "MmAddMemoryRegion\n"));
  ...
  if (Type == EfiConventionalMemory) {
    MmInternalFreePagesEx (AlignedMemBase, TRUNCATE_TO_PAGES ((UINTN)MemLength), TRUE, TRUE);
  } else {
    ConvertMmMemoryMapEntry (EfiRuntimeServicesData, AlignedMemBase, TRUNCATE_TO_PAGES ((UINTN)MemLength), TRUE, TRUE);
  }
  ...
  for (UINTN Index = 0; Index < mMmramRangeCount; Index++) {
    ...
    while (Link != &gMemoryMap) {
      ...
      if (Start < Entry->Start) {
        ...
        DEBUG ((DEBUG_INFO, "%a - Found starting free entry Start: 0x%lx, End: 0x%lx within MMRAM %p - %p (State %x).\n",
          __func__,
          Start,
          Entry->Start - 1,
      ...
      if ((Entry->Start >= Start) && (Entry->End <= End)) {
        ...
        DEBUG ((DEBUG_INFO, "%a - Including allocated Start: 0x%lx, End: 0x%lx, Type: 0x%x, IsSupervisorPage: %d\n",
          __func__,
          Entry->Start,
          Entry->End,
          Entry->Type,
    ...
    if (Start <= End) {
      ...
      DEBUG ((DEBUG_INFO, "%a - Including ending free entry Start: 0x%lx, End: 0x%lx within MMRAM %p - %p (State %x).\n",
        __func__,
        Start,
        End,
        MmramEntry->PhysicalStart,
  ...
  *MmHobSize = TotalHobSize;
  ...
  for (UINTN Index = 0; Index < mMmramRangeCount; Index++) {
    ...
    while (Link != &gMemoryMap) {
      ...
      if ((Entry->Start >= Start) && (Entry->End <= End)) {
        // This entry is inside the mmram region
        DEBUG ((DEBUG_INFO, "%a - Including Start: 0x%lx, End: 0x%lx, Type: 0x%x, IsSupervisorPage: %d\n",
          __func__,
          Entry->Start,
          Entry->End,
          Entry->Type,

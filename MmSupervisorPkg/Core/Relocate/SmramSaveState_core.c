/** @file
Core-build specialization for the SMRAM Save State Map services.

Houses Core's variants of GetSmiHandlerSize and InstallSmiHandler plus the
Core-only SmmReadSaveState and ProcessUserSaveStateAccess paths and their
supporting locals.  The Init-side equivalents live in SmramSaveState_init.c;
truly shared state lives in SmramSaveState.c.

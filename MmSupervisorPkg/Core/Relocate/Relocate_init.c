/** @file
  Init-build specialization for the SMM Entry Vector relocation logic.
...
//
// TODO: This should not be here.  Init's Common/MpService.h has these
// declarations commented out (see Slice 7e for the planned consolidation), so
// we re-declare what SetupSmiEntryExit / LockMmCoreBeforeExit need from the
// Init MP-service compilation unit.
//
...
/**
  ...
  Init variant: takes a (MmHobStart, RemainingSize) pair so the caller can
  bookend HOB construction with the lock step.

  @param[in] MmHobStart       Base of the supervisor's HOB region.
  @param[in,out] RemainingSize  In: bytes still available in the HOB region;
                                Out: updated remainder after PrepareMmSupervisorHobs.
 **/
VOID
EFIAPI
  ...
  )
{
  ...
  Status     = PrepareMmSupervisorHobs (MmHobStart, &BufferSize);
  if (EFI_ERROR (Status)) {

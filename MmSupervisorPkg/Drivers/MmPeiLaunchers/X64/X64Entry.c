  ...
  )
{
  ...
  DEBUG ((DEBUG_ERROR, "MmIplPeiX64 - PageFaultHandler: Cr2 - %lx\n", PFAddress));
...
/**
  The X64 entrypoint is used to process MM core in long mode then
  ...
  )
{
  ...
  // MU_CHANGE: Directly load MM Core from supplied entrypoint and HobStart pointers.
  //
  // The one and only job for this module: Invoke MM core entry point in X64.
  //
  ...
  //
  // Finish to initialize MM core in X64, and return to 32-bit mode.
  //
  AsmDisablePaging64 (

  ...
  )
{
  ...
  Prot = MmFindUserProtocolInterface (Handle, Protocol, Interface);
  if (Prot != NULL) {
    ProtEntry = Prot->Protocol;
  ...
  )
{
  ...
  if (Function == NULL) {
    ...
    //
    ProtEntry = MmFindUserProtocolEntry ((EFI_GUID *)Protocol, FALSE);
    if (ProtEntry != NULL) {
  ...
  //
  ProtEntry = MmFindUserProtocolEntry ((EFI_GUID *)Protocol, TRUE);
  if (ProtEntry != NULL) {
    //
    ...
    //
    MmAllocateUserPool (EfiRuntimeServicesData, sizeof (PROTOCOL_NOTIFY), (VOID **)&ProtNotify);
    if (ProtNotify != NULL) {

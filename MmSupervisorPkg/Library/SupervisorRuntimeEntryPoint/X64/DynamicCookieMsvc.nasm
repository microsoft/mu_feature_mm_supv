...
extern ASM_PFX(SmiRendezvousWrapper)
...
ASM_PFX(_ModuleEntryPoint):
  ...
  ; First check if the seed is already set
  lea rbx, [rel ASM_PFX(__security_cookie)]  ; load the address of __stack_check_guard into rbx
  cmp qword [rbx], 0                     ; is __stack_chk_guard already set?
  jne c_entry                           ; if it is, skip setting it again
...
c_entry:
  ...
  pop rcx
  pop rbx
  jmp ASM_PFX(SmiRendezvousWrapper)

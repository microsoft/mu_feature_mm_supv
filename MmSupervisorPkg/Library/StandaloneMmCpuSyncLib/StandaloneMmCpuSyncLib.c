/** @file
  STANDALONE MM CPU Sync lib implementation.

  The lib provides 3 sets of APIs:
  1. ContextInit/ContextDeinit/ContextReset:

    ContextInit() is called in driver's entrypoint to allocate and initialize the STANDALONE MM CPU Sync context.
    ContextDeinit() is called in driver's unload function to deinitialize the STANDALONE MM CPU Sync context.
  ...
  2. GetArrivedCpuCount/CheckInCpu/CheckOutCpu/LockDoor:
    When SMI happens, all processors including BSP enter to STANDALONE MM mode by calling CheckInCpu().
...
/**
  ...
  @param[in,out]  Context     Pointer to the SMM CPU Sync context object to be de-initialized.

...
__These functions will not work in a non-privileged context even if the build errors are resolved__ but can be resolved
by either filing a github issue that extends the syscall interface to support the needed function or by finding an alternative

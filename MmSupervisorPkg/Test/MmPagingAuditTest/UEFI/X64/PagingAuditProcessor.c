  ...
  )
{
  ...
  INTN        LowBitPositionMask;
  INTN        HighBitPositionMask;
  UINTN       NumberOfTseg;
  UINTN       Index;
  UINTN       BitIndex;
  UINTN       RecordIndex;
  ...
  if (!StandardSignatureIsAuthenticAMD ()) {
    ...
    ASSERT (LowBitPositionMask > 0 && HighBitPositionMask > 0);
    ...
    NumberOfTseg = 1;
    for (BitIndex = LowBitPositionMask + 1; BitIndex <= (UINTN)HighBitPositionMask; BitIndex++) {

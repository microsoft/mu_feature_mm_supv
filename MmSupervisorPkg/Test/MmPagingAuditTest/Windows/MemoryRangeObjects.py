...
class MemoryRange(object):
    ...
    def __init__(self, record_type, *args, **kwargs):
        ...
        if self.RecordType not in ("TSEG", "MemoryMap", "LoadedImage", "SmmLoadedImage", "PDE", "GDT", "IDT", "PTEntry", "MAT", "GuardPage", "Bitwidth",
            "SmiEntry", "SmmSaveState", "SupervisorStack", "UserStack", "SupervisorCommBuffer", "UserCommBuffer", "UnblockedRegion"):
        ...
        if self.RecordType in ("LoadedImage", "SmmLoadedImage"):
            self.LoadedImageEntryInit(int(args[0], 16), int(args[1], 16), args[2], args[3])
        elif self.RecordType in ("MemoryMap", "TSEG", "MAT"):
            self.MemoryMapEntryInit(*(int(arg, 16) for arg in args))
        elif self.RecordType in ("PDE", "IDT", "SupervisorStack", "UserStack", "SupervisorCommBuffer", "UserCommBuffer"):
            self.MiscEntryInit(int(args[0], 16), int(args[1], 16), self.RecordType)
        elif self.RecordType in ("GDT", "SmiEntry", "SmmSaveState"):
            self.MiscEntryInit(int(args[0], 16), int(args[1], 16), '-'.join([self.RecordType, args[2]]))
        ...
        elif self.RecordType in ("UnblockedRegion"):
            self.LoadedImageEntryInit(int(args[0], 16), int(args[1], 16), self.RecordType, args[3])
...
""" % (self.GetMemoryTypeDescription(), self.PhysicalStart, self.PhysicalEnd, self.VirtualStart, self.NumberOfPages, self.Attribute, self.PhysicalSize)
    ...
    #
    def LoadedImageEntryInit(self, Base, Size, Name, OwnerGuid):
        self.PhysicalStart = Base
        self.PhysicalSize = Size
        self.ImageName = Name
        self.OwnerGuid = OwnerGuid

    def MiscEntryInit(self, Base, Size, Name):
        self.PhysicalStart = Base
        self.PhysicalSize = Size
        self.ImageName = Name
